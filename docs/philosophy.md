...
**Pyfallow sits next to infrastructure gates, but for code instead of infra.** Where an infrastructure gate enforces "declared state matches runtime state," pyfallow enforces "what the AI agent claimed it edited is structurally consistent in the repository graph."
...
- **Pyfallow does not have memory between invocations.** Same inputs → same outputs. No "well last time you said X" reasoning.
...
- ADR 0007 — pyfallow as deterministic harness
